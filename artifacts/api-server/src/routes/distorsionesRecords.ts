...
const DISTORSION_KEYS = [
  ...
  "filtro-mental",
  "descalificacion-positivo",
  "conclusiones-precipitadas-lectura",
  "conclusiones-precipitadas-anticipacion",
  "magnificacion-catastrofizacion",
  "razonamiento-emocional",
  "los-deberias",
  "etiquetacion",
  "personalizacion",
] as const;
type DistorsionKey = typeof DISTORSION_KEYS[number];
const DISTORSION_KEY_SET = new Set<string>(DISTORSION_KEYS);

const EDIT_WINDOW_MS = 48 * 60 * 60 * 1000; // 48 horas

interface ItemValue { key: DistorsionKey; value: number }
...
function sanitizeItems(input: unknown): ItemValue[] | { error: string } {
  ...
  for (const raw of input) {
    ...
    if (typeof k !== "string" || !DISTORSION_KEY_SET.has(k)) {
    ...
    out.push({ key: k as DistorsionKey, value: Math.round(n) });
  }
...
registerPsiRecordRoutes(router, {
  table: distorsionesRecordsTable,
  auditName: "DISTORSIONES",
  targetTable: "distorsiones_records",
  taskKeys: psiRecordTaskKeys("distorsiones"),
  mapBody: (b) => ({ items: pArr(b.items) as any, notas: typeof b.notas === "string" ? b.notas.slice(0, 4000) : null }),
});

// GET /api/distorsiones/mine — paciente lista sus propios registros
router.get("/mine", requirePaciente, async (req: any, res) => {
  const rows = await db.select().from(distorsionesRecordsTable)
    .where(and(eq(distorsionesRecordsTable.pacienteId, req.session.userId), isNull(distorsionesRecordsTable.psicologoId)))
    .orderBy(desc(distorsionesRecordsTable.createdAt));
  res.json(rows.map(r => ({
    ...r,
...
// POST /api/distorsiones/mine — paciente crea un nuevo registro (repetible)
router.post("/mine", requirePaciente, async (req: any, res) => {
  ...
  if (b.assignmentId !== undefined && b.assignmentId !== null) {
    ...
    if (row.key !== "distorsiones-realidad") {
  ...
  const [row] = await db.insert(distorsionesRecordsTable).values({
...
// PATCH /api/distorsiones/:id — paciente dueño edita (sólo dentro de 48h)
router.patch("/:id", requirePaciente, async (req: any, res) => {
  ...
  const [existing] = await db.select().from(distorsionesRecordsTable)
    .where(eq(distorsionesRecordsTable.id, id)).limit(1);
  ...
  const [row] = await db.update(distorsionesRecordsTable)
    .set(update as any)
    .where(eq(distorsionesRecordsTable.id, id))
    .returning();

  await logAudit({
    actorId: req.session.userId,
    actorName: null,
    action: "UPDATE_DISTORSIONES",
    targetTable: "distorsiones_records",
    targetId: row.id,
    ipAddress: getIp(req),
...
// DELETE /api/distorsiones/:id — paciente dueño borra (sólo dentro de 48h)
router.delete("/:id", requirePaciente, async (req: any, res) => {
  ...
  await db.delete(distorsionesRecordsTable).where(eq(distorsionesRecordsTable.id, id));

  await logAudit({
    actorId: req.session.userId,
    actorName: null,
    action: "DELETE_DISTORSIONES",
    targetTable: "distorsiones_records",
    targetId: id,
    ipAddress: getIp(req),
    details: null,
...
// GET /api/distorsiones — admin/psicólogo (psi sólo ve a sus pacientes asignados)
router.get("/", requireAdminOrPsi, async (req: any, res) => {
  ...
  if (pacienteId !== null && Number.isInteger(pacienteId)) {
    filters.push(eq(distorsionesRecordsTable.pacienteId, pacienteId));
  }
  ...
  if (req.session.userRole === "psicologo") {
    ...
    const rows = await db.select({
      ...
      .from(taskAssignmentsTable)
      .innerJoin(therapeuticTasksTable, eq(therapeuticTasksTable.id, taskAssignmentsTable.taskId))
      .where(eq(therapeuticTasksTable.key, "distorsiones-realidad"));
    ...
    filters.push(inArray(distorsionesRecordsTable.pacienteId, allowedIds));
  }
...
// GET /api/distorsiones/:id — admin/psi/owner pueden leer
router.get("/:id", requireAuth, async (req: any, res) => {
  const id = parseInt(req.params.id);
  if (!Number.isInteger(id)) { res.status(400).json({ error: "ID inválido" }); return; }
  const [row] = await db.select().from(distorsionesRecordsTable)
    .where(eq(distorsionesRecordsTable.id, id)).limit(1);
  ...
  if (role === "admin") {
    ...
    const rels = await db.select({
      ...
      .innerJoin(therapeuticTasksTable, eq(therapeuticTasksTable.id, taskAssignmentsTable.taskId))
      .where(and(
        eq(therapeuticTasksTable.key, "distorsiones-realidad"),
