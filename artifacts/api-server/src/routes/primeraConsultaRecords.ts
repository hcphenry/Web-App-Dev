...
registerPsiRecordRoutes(router, {
  ...
  taskKeys: psiRecordTaskKeys("primera-consulta"),
...
router.post("/mine", requirePaciente, async (req: any, res) => {
  ...
  const [row] = await db.insert(primeraConsultaRecordsTable).values({
  ...
  await logAudit({
    ...
    actorName: null,
    action: "CREATE_PRIMERA_CONSULTA",
    targetTable: "primera_consulta_records",
    targetId: row.id,
    ipAddress: getIp(req),
...
router.get("/", requireAdminOrPsi, async (req, res) => {
  ...
  if (pacienteId !== null && Number.isInteger(pacienteId)) {
    filters.push(eq(primeraConsultaRecordsTable.pacienteId, pacienteId));
  }
  const rows = await db.select().from(primeraConsultaRecordsTable)
...
router.get("/:id", requireAuth, async (req: any, res) => {
  ...
  const [row] = await db.select().from(primeraConsultaRecordsTable)
